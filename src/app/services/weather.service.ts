...
export class WeatherService {
  ...
  getForecastData(city: string): Observable<ForecastData> {
    ...
    return this.http
      .get<any>(this.forecastUrl, { params })
      .pipe(
  ...
  getAirQualityData(lat: number, lon: number): Observable<any> {
    ...
    return this.http
      .get<any>(this.airQualityUrl, { params })
      .pipe(
        catchError((error) => {
  ...
  private transformOpenWeatherData(data: any): WeatherData {
    return {
      ...
      current: {
        ...
        feelslike_f: Math.round(((data.main.feels_like * 9/5) + 32) * 100) / 100,
        ...
        dewpoint_f: Math.round(((this.calculateDewPoint(data.main.temp, data.main.humidity) * 9/5) + 32) * 1000) / 1000,
  ...
  private transformForecastData(data: any): ForecastData {
    const forecastDays = data.list.filter((item: any, index: number) => index % 8 === 0).slice(0, 5);
      ...
      forecast: {
        forecastday: forecastDays.map((day: any) => ({
          ...
          day: {
            ...
            maxtemp_f: Math.round(((day.main.temp_max * 9/5) + 32) * 100) / 100,
            mintemp_c: day.main.temp_min,
            mintemp_f: Math.round(((day.main.temp_min * 9/5) + 32) * 100) / 100,
            ...
            totalprecip_in: Math.round((day.rain?.['3h'] || 0) * 0.039 * 100) / 100,
            totalsnow_cm: 0,
